...
"""
Paged Flash Attention Implementation
...
def paged_flash_attention_kernel(
    ...
    head_mask = (jnp.arange(P) < K)
    q_RP = pl.load(q_ref, (slice(None), slice(None)), mask=head_mask[None, :], other=0.0)
    ...
    def _compute(start_kv, m_prev_R, l_prev_R, o_prev_RP, should_mask):
        page_start = start_kv * pages_per_block
        page_slice = pl.dslice(page_start, pages_per_block)
        page_indices = pl.load(page_ref, page_slice)
        k_CP = pl.load(k_ref, (page_indices, slice(None), slice(None)), mask=head_mask[None, None, :], other=0.0).reshape((C, P))
        ...
        v_CP = pl.load(v_ref, (page_indices, slice(None), slice(None)), mask=head_mask[None, None, :], other=0.0).reshape(C, P)
    ...
    def body(start_kv, carry):
        ...
        return jax.lax.cond(
            should_compute,
            lambda: _compute(start_kv, m_prev_R, l_prev_R, o_prev_RP, should_mask),
            lambda: carry
        )

    m_R, l_R, o_RP = jax.lax.fori_loop(0, pl.cdiv(page_ref.shape[0], pages_per_block), body, (m_R, l_R, o_RP))

    l_R = jnp.where(l_R == 0.0, 1, l_R)
    o_RP /= l_R[:, None]
    pl.store(out_ref, (slice(None), slice(o_RP.shape[-1])), o_RP.astype(out_ref.dtype), mask=head_mask[None, :])


@partial(jax.jit, static_argnames=['scale', 'mask_mod', 'scores_mod', 'block_size_q', 'block_size_kv', 'page_size', 'pages_per_block', 'num_warps', 'num_stages'])
def paged_flash_attention(
    ...
    L, H, K = q_LHK.shape
    assert block_size_kv == page_size * pages_per_block, "block_size_kv must equal page_size * pages_per_block"
    N = page_indices_N.shape[0]
    R = min(L, block_size_q)
    C = block_size_kv
    P = pl.next_power_of_2(K)

    QB = pl.cdiv(L, R)
    max_blocks = pl.cdiv(N, pages_per_block)
    KB = max_blocks
    ...
    kernel = partial(
        ...
        mask_fn=_mask_mod,
        scores_fn=_scores_mod
    )

    in_specs = [
        pl.BlockSpec((R, None, P), lambda i, j: (i, j, 0)),  # q_ref
        pl.BlockSpec((N, page_size, None, P), lambda _, j: (0, 0, j, 0)),  # k_ref
        pl.BlockSpec((N, page_size, None, P), lambda _, j: (0, 0, j, 0)),  # v_ref
        pl.BlockSpec((N,), lambda _, j: (0,)),  # page_ref
        pl.BlockSpec((QB, KB), lambda i, j: (0, 0)),  # block_mask_ref
    ]
    ...
    block_mask = jnp.ones((QB, KB), dtype=jnp.int8)
    if _mask_mod is not None:
        # For paged attention, we need to adjust the mask computation
        effective_M = KB * C  # Effective sequence length
        block_mask = sparsify_block_mask(_mask_mod, L, effective_M, R, C)
    ...
    )(q_LHK, k_NSHK, v_NSHK, page_indices_N, block_mask)
